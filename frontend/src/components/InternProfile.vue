...
<div class="mx-auto max-w-7xl ">
  ...
  <div v-for="profile in profiles.slice(6,7)" v-bind:key="profile.id" class="py-6 mx-auto">
    <div class="sm:grid grid-cols-3 lg:grid-cols-4 gap-6 mx-auto shadow-sm bg-brand-red-light-2">
      ...
      <img class="object-cover w-full h-96 md:h-72 rounded" v-bind:src="profile.picture" v-bind:alt="profile.alt" />
  
      <div class="grid-cols-2 sm:col-span-2 lg:col-span-3 py-4 text-left">
        <h1 class="text-2xl md:text-3xl font-bold text-brand-gray-dark-1">{{profile.full_name}}</h1>
        <h3 class="py-2 text-lg md:text-xl text-brand-gray-dark-1 font-medium">{{profile.stack.join(',')}}</h3>
